...
- **隐私遮蔽** — 隐私分类（医疗、人情、理财）金额自动遮蔽为 ••••，首页支出 / 收入 / 结余三项各自独立遮蔽开关
- **账户管理** — 增删改查账户，设置默认账户，关联记录检查
- **分类管理** — 大类 / 小类增删改查、拖拽排序、可调分栏、隐私标记
- **分类结构** — 8 个支出大类 + 4 个收入大类，各含多个小类，首启自动初始化
- **自动记账** — 三种触发类型（每周多选 / 每月多选 / 每隔 N 天）+ 启动时追赶引擎，App 离线期间漏掉的触发会在下次启动时补齐
- **空状态占位 UI** — 首页 / 自动记账 / 账户列表无数据时显示引导占位
- **过渡动画** — 设置子页 Slide 转场 + 底部 Tab Crossfade + 记录列表 itemPlacement
- **CSV 导入导出** — 全部记录一键导出 CSV，也支持从 CSV 导入历史记录（按名字匹配分类 / 账户，非法行整行跳过）
- **本地通知提醒** — Android / iOS 每日定时提醒记账，最多 3 条、相邻不少于 1 小时，Desktop 隐藏入口

### 计划中

- 中国法定工作日 / 节假日触发类型
- 嵌入字体（Noto Sans SC / Inter）
- iCloud / Google Drive / 本地备份恢复
